          ...
          <template #header>
            <div class="flex justify-content-between">
                <div class="flex align-items-center">
                    ...
                    <span class="font-bold white-space-nowrap ml-2">ผัก</span>
                </div>
...
data() {
  ...
  methods: {
     ...
     close: func.close,
      newDetails(){
        this.$router.push({ path: `/StockDetailsVetgetable`});
      },
      goDetails(data){
          this.$router.push({ path: `/StockDetailsVetgetable/${data.id}`});
      },
      getWorklistStock(){
        this.show()
        axios.get(`${import.meta.env.VITE_API_URL}/stocks/VT`)
          .then(response => {
